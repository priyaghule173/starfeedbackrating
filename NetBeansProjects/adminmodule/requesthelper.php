...
if (isset($_REQUEST['request'])) {
    ...
    $result = dwqueryexec(__FUNCTION__, $sql);
    $dwempid = $thisemployer['dwempid'];
$sql = " SELECT  * FROM  `request-helper` WHERE `dwempid` = '" . $dwempid . "' ORDER BY `reqhelperid` DESC LIMIT 1";
   

$result = dwqueryexec(__FUNCTION__, $sql);

    $currentrequest = mysqli_fetch_all($result, MYSQLI_ASSOC);
    count($currentrequest);
     ...
     $message .="<table  class='table-bordered' width=100% border=1>";
      $message .="<thead>";
                                ...
                                $message.="<tr>";
                                    ...
                                    // echo $dwempid;
                                     //cho $currentemp[0]['dwempid'];
                                      $message.='<td><a href='.site_url() . '/viewrequesthelper?v_reqhelperid='.$currentrequest[0]['reqhelperid'].'>'.$currentrequest[0]['area'].'</a></td>';
...
$cc_mai=array("priyaghule173@gmail.com","punamnavale26@gmail.com","dev2@sandeepsuryavanshi.com");
 $id=(createTicket($message,$subj,$typ,$ema,$prio,$stat,$cc_mai));    
   ...
   /* if ($GLOBALS['conn']->error) {
        //Error
        echo "<script>alert('Please Enter data again'); document.location='" . site_url() . "/requesthelper';</script>";
    } else {
        echo "<script>alert('We Accept Your Request, we will contact you in next 24 hours'); document.location='" . site_url() . "/requesthelper';</script>";
    }*/
}
?>
...
<?php get_header(); ?>
    <body>
        <div class="container-fluid">
            <h1 class="text-center"><b>Request Helper</b></h1>
            <div class="col-md-12">
                <form class="form-horizontal demo-form" enctype="multipart/form-data" name="contract Form" data-parsley-validate="" method="POST" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
                    <div class="form-group">
                        <label class="control-label col-md-4">Select Area Where You want a Helper</label>
                        <div class="col-md-4">
...
?>
                     ...
                     <div style="text-align: center;">
                                               ...
                                               Tell us little bit more information & 
                                               <strong class='green'>
...
<!--                              <label class="control-label">Tell us little bit more information & we will quickly get back to you </label>-->
                     ...
                     <div class="form-group">
                        ...
                        <div class="col-md-4">
                            <div class="checkbox">
                                ...
                                <label><input type="checkbox" name="v_worktype[]" value="OT"> Other</label>
                            </div> 
                        </div>
                    ...
                    <div class="form-navigation">
                        <input  type="reset" id="Reset" name="Reset" value="Reset" class="btn  pull-left" >

                        <input type="submit" id="submit" name="request" value="Request" class="btn  pull-right">
                    </div>
                    </div>
     </div>
                </form>
